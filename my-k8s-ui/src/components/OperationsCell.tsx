...
export default function OperationsCell({ name, status, apiRef }: Props) {
  ...
  useEffect(() => {
    if (opId && op?.phase === 'idle' && pendingAction) {
      ...
      qc.setQueryData<{name:string;status:AppStatus;namespace:string}[]>(['apps'], old =>
        old?.map(a => a.name === name ? { ...a, status: nextStatus } : a) ?? []
      );
  ...
  return (
    <Box
      sx={{
        ...
        justifyContent: processing ? 'flex-start' : 'center',
      }}
    >
      <Stack
        direction="row"
      ...
      >
        {!processing && status === 'stopped' && (
          <>
            <Button size="small" variant="contained" onClick={() => handleOp('start')}>
              起動
            </Button>
            <Button size="small" onClick={() => handleOp('uninstall')}>
              アンインストール
            </Button>
          </>
        )}
        ...
        {processing && (
          <Chip label={phaseLabel} size="small" color="primary" />
        )}
      </Stack>
